...
TOPO is now in **local alpha**. The canonical claim lifecycle, local SQLite store, CLI and a usable Tauri desktop memory manager are working. The desktop, CLI and interoperability adapters share the same local store rather than maintaining parallel memory.
